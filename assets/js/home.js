(function () {
  ...
  async function loadProjects() {
    ...
    container.innerHTML = `<div class="empty-state">Loading GitHub projects...</div>`;
    ...
    try {
      ...
      const filtered = repos
        ...
        .sort((a, b) => {
          const scoreA = (a.stargazers_count || 0) * 3 + (a.forks_count || 0) + (a.description ? 1 : 0);
          const scoreB = (b.stargazers_count || 0) * 3 + (b.forks_count || 0) + (b.description ? 1 : 0);
          return scoreB - scoreA;
        })
        .slice(0, MAX_REPOS);

      if (!filtered.length) {
        container.innerHTML = `<div class="empty-state">No public repositories found yet.</div>`;
        return;
      }

      container.innerHTML = filtered.map((repo) => {
        const topics = (repo.topics || []).slice(0, 4).map((topic) => `<span class="pill">${escapeHtml(topic)}</span>`).join("");
          ...
          <article class="card project-card">
            <div class="project-top">
              <div>
                ...
                <div class="repo-meta">${language} | Updated ${escapeHtml(updated)}</div>
              </div>
              <div class="repo-meta">${stars} stars</div>
            </div>
            <p>${escapeHtml(repo.description || "Public GitHub project.")}</p>
      ...
      console.error(error);
      container.innerHTML = `<div class="empty-state">Could not load GitHub projects automatically.</div>`;
    }
  }
